...
const funbonuscasRef = db.collection('rtp');
...
function deleteDocumentByIdAndAnimateRow() {
  if (deleteDocumentId && deleteDocumentRow) {
    const docRef = db.collection('rtp').doc(deleteDocumentId);
...
function compileModifyModal(modifydocId){
  ...
  db.collection('rtp').doc(modifydocId).get()
  .then((doc) => {
    if (doc.exists) {
      const data = doc.data();
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          if(row){
            ...
            var docRef = db.collection("rtp").doc(modifydocId);
            docRef.update({
              data: mytime,
              esito: varEsito,
