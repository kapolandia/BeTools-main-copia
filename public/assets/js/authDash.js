...
function deleteDocumentByIdAndAnimateRow() {
  if (deleteDocumentId && deleteDocumentRow) {
    const docRef = db.collection('funbonuscas').doc(deleteDocumentId);
...
function compileModifyModal(modifydocId){
  ...
  db.collection('funbonuscas').doc(modifydocId).get()
  .then((doc) => {
    if (doc.exists) {
      const data = doc.data();
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          if(row){
            ...
            var docRef = db.collection("funbonuscas").doc(modifydocId);
            docRef.update({
              data: mytime,
              esito: varEsito,
