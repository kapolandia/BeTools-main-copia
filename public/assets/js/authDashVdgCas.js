...
const funbonuscasRef = db.collection('vdgcas');
...
function deleteDocumentByIdAndAnimateRow() {
  if (deleteDocumentId && deleteDocumentRow) {
    const docRef = db.collection('vdgcas').doc(deleteDocumentId);
...
function compileModifyModal(modifydocId){
  ...
  db.collection('vdgcas').doc(modifydocId).get()
  .then((doc) => {
    if (doc.exists) {
      const data = doc.data();
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          if(row){
            ...
            var docRef = db.collection("vdgcas").doc(modifydocId);
            docRef.update({
              data: mytime,
              esito: varEsito,
